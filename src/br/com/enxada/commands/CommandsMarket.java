	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
			...
			if (args.length == 1) {
				...
				if (id != 0) {
					if (!market.itemExist(id)) {
						market.addItem(id);
						player.sendMessage(
								Util.chat("&eItem &c[&f" + id + "&c] &9" + Util.getItemName(id) + " &aAdicionado!"));
						return true;
					} else {
				...
				if (Util.isNumber(args[1])) {
					...
					if (id != 0) {
						if (!market.itemExist(id)) {
							...
							player.sendMessage(Util.chat("&eItem &c[&f" + id + "&c] &9" + Util.getItemName(id)
									+ " &aAdicionado! \n&ePreço: &f" + price));
							return true;
						} else {
							player.sendMessage(Util.chat("&cItem já está registrado!"));
						...
						player.sendMessage(
								Util.chat("&cParâmetros inválidos: /additem [id] ou <nome do item> <preço>"));
						return true;
					}
				}
		...
		if (cmd.getName().equalsIgnoreCase(cmdAtualizarItem)) {
			...
			if (args.length == 2) {
				...
				if (Util.isNumber(args[1])) {
					...
					if (id != 0) {
						if (market.itemExist(id)) {
							...
							player.sendMessage(Util.chat(
									"&eItem &9" + Util.getItemName(id) + "&a Atualizado\n&ePreço atual: &f" + value));
							return true;
					...
					player.sendMessage(
							Util.chat("&cParâmetros inválidos: /atualizaritem [id] ou <nome do item> <preço>"));
					return true;
				}
		...
		if (cmd.getName().equalsIgnoreCase(cmdListarItens)) {
			...
			if (args.length == 0) {
				...
				player.sendMessage(Util.chat("&f----------&6[Lista]&7 (1/" + Util.mapKeyQuantity(map)
						+ ")&f--------------------"
						+ "\n&7Use /listaritems [n] para ir para pagina n \n&f----&9NOME &f----&aPREÇO &f----&cID"));

				listItem = (List<ItemStack>) map.get(0);
